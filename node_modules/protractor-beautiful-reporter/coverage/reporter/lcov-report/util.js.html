...
function stringifyHtml(source){
    ...
    var res=source.split(/^/gm).map(function(line) {
        ...
        return "";
    }).join('') || <span class="branch-1 cbranch-no" title="branch not covered" >'""';</span>
&nbsp;
...
&nbsp;
        if (options.prepareAssets) {
            ...
            <span class="missing-if-branch" title="if path not taken" >I</span>if (options.customCssInline) {
<span class="cstat-no" title="statement not covered" >                cssInsert += ` &lt;style type="text/css"&gt;${options.customCssInline}&lt;/style&gt;`;</span>
            }
&nbsp;
            //copy assets
...
<span class="cstat-no" title="statement not covered" >                    addMetaData(test, baseName, options);</span>
                }, 200);
...
<span class="cstat-no" title="statement not covered" >        console.error('Could not save JSON for data: ' + test);</span>
    }
}
&nbsp;
